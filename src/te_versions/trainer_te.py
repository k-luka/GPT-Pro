...
class Trainer:
    def __init__(self, model, train_data_root, val_data_root, config: TrainerConfig, tokenizer=None, wandb_run=None):
        self.config = config
        self.model = model
        ...
        self.fp8_recipe = DelayedScaling(fp8_format=self.fp8_format, amax_history_len=16, amax_compute_algo="max")
    ...
    def _generate(self):
        ...
        with FSDP.summon_full_params(self.model, writeback=False, rank0_only=True):
            if self.rank == 0:
                ...
                context = torch.tensor(gen_prompt, dtype=torch.long, device=self.config.device)
                with torch.no_grad():
    ...
    def train(self, resume_from_checkpoint=None):
        ...
        if resume_from_checkpoint is not None:
            ...
            if self.rank == 0:
                print(f"---| Resuming training from step {start_step} until {self.config.max_steps} |---")
        ...
        for step in range(start_step, self.config.max_steps+1):
            ...
            t1 = time.time()
            dt = t1 - t0
            tps = (self.train_loader.B * self.train_loader.T * self.config.grad_accum_steps * self.world_size) / dt

            if self.rank == 0:
                # report to wandb
                if self.wandb_run is not None:
                    self.wandb_run.log({"train loss": float(loss), "tokens/sec": float(tps), "train step time (ms)": dt*1000})
                # print train loss and stats to console
                if step % self.config.logging_steps == 0:
                    print(f"Step: {step} | loss: {loss:.6f} | dt: {dt*1000:.4f} ms | tokens/sec: {tps:.4f}")
            # eval loss and report it
            val_loss = None
            if step % self.config.eval_interval == 0:
                val_loss = estimate_loss(self.model, self.val_loader, self.config.eval_steps, self.config.device)
                ...
                if self.rank == 0:
                    if self.wandb_run is not None:
                        self.wandb_run.log({"val loss": val_loss, "HellaSwag accuracy": hella_acc})
