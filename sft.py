...
def load_pretrained_checkpoint(
    model, optimizer, checkpoint_path: str, device: torch.device
):
...
def save_sft_checkpoint(
    model, optimizer, step: int, best_val_loss: float, run_name: str, is_best: bool
):
...
def main(cfg: DictConfig):
    ...
    for step in range(start_step + 1, cfg.training.max_steps + 1):
        ...
        if step % cfg.training.eval_interval == 0:
            ...
            if master_rank:
                print(
                    f"step {step:6d} | val_loss {val_loss:.6f} | best {best_val_loss:.6f}"
                )
