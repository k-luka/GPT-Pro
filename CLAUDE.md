...
- **Multihead Latent Attention (MLA)** with RoPE
...
│
├── config/
│   ├── config_pretrain.yaml     # Main pretraining config (model + training params)
...
├── src/
│   ├── models/
│   │   ├── gpt_te.py            # PRIMARY model: MLA + MoE with Transformer Engine
...
│   └── utils/
│       ├── helpers.py           # Param counting, FLOP estimation, RoPE, checkpoint utils
│       └── optimizers.py        # Muon optimizer + DualOptimizer
│
├── scripts/
...
| `config/config_pretrain.yaml` | All hyperparameters — **first place to look when adjusting a run** |
| `src/datasets/dataloader.py` | Distributed binary shard dataloader |
| `src/utils/optimizers.py` | Muon + DualOptimizer (AdamW for embeddings/norms, Muon for linear layers) |
...
**GQA (Grouped Query Attention)** — lines ~13–52
- 32 query heads, 8 KV heads (configurable via `n_heads`, `n_kv_heads`)
- Per-head RMSNorm on Q and K before RoPE
- `scaled_dot_product_attention` with causal mask

**Gate (MoE Router)** — lines ~140–162
- Top-k expert selection with sigmoid scoring + bias correction
- Bias updated from global expert token counts (no auxiliary loss)

**SharedExpert** — lines ~116–136
- Dense FFN applied to all tokens unconditionally
- SiLU gating; hidden size aligned to 256

**MoE** — lines ~166–258
- Single-GPU MoE: shared experts + routed experts via TE `GroupedLinear`
...
**ExpertParallelMoE** — lines ~261–450
- Multi-GPU expert parallelism: routed experts sharded across ranks
- All-to-all dispatch → local expert compute → all-to-all return

**Block** — lines ~454–486
- Pre-norm: `LN1 → GQA`, `LN2 → MoE`, with residual connections
- Uses `torch.compile`d attention during training

**GPT** — lines ~490+
- Token + position embeddings (weight-tied with output projection)
- Pre-computed RoPE sin/cos buffers
- `generate()` method for autoregressive text generation
...
- No separate auxiliary loss term — simpler than the original DeepSeek-V3 approach
...
experiment:
  ...
  run_name: "LM_Neo"     # W&B run name + checkpoint directory name
...
training:
  ...
  batch_warmup_steps: 1000   # Steps to ramp up from batch_size=1 to full
  max_steps: 160000
...
# Uncomment to resume:
# resume_checkpoint: output/checkpoints/<run_name>/best_val
```
...
- TE `FP8GlobalStateManager` with `HYBRID` format and delayed scaling (`amax_history_len=16`)
- BF16 model parameters stored; FP8 used only for GEMM operations

### Optimizer (`src/utils/optimizers.py`)
- **DualOptimizer**: applies different optimizers to different param groups
  - **AdamW**: embeddings, norms, biases
  - **Muon**: all linear weight matrices (uses Newton-Schulz orthogonalization)
...
- Starts with `grad_accum_steps=1`, ramps to configured value over `batch_warmup_steps`
...
# 2 nodes × 4 GPUs = 8 GPUs total, c10d backend
```

### Resuming a Run
```bash
torchrun --nproc-per-node=8 pretrain.py resume_checkpoint=output/checkpoints/LM_Neo/best_val
```
...
- Do not accidentally assign embedding/norm parameters to the Muon group — Muon is designed for weight matrices only.
- `DualOptimizer` splits params into `muon_params` and `adamw_params`; verify any new parameters are in the right group.
...
**Change model size:** Edit `model.n_embd`, `model.n_layers`, `model.n_heads`, etc.
...
**Add a new optimizer parameter group:** Modify `DualOptimizer` in `src/utils/optimizers.py` and the param-group logic in `trainer_te.py`.
